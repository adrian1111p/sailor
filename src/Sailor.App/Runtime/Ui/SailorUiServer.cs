...
<div class="header">
  ...
  <div class="statusBox"><span id="status" class="statusWarn">loading</span> <span id="source" class="small"></span><span id="controlBadge" class="badge">read-only</span><span class="badge">1s refresh</span><span class="badge">max 2 strategies</span><span id="activeStrategies" class="small"></span><span id="actionStatus" class="actionStatus"></span></div>
</div>
<div class="main">
...
function action(msg, bad=false){ const el=document.getElementById('actionStatus'); el.textContent=msg || ''; el.className = bad ? 'actionStatus neg' : 'actionStatus'; if(msg) setTimeout(()=>{ if(el.textContent===msg) el.textContent=''; }, 3500); }
